...
struct Node {
    ...
    Node() : x(0), y(0), prev(nullptr), next(nullptr), ring_id(-1), generation(0) {}
    Node(double x_, double y_, int ring_id_ = -1) 
        : x(x_), y(y_), prev(nullptr), next(nullptr), ring_id(ring_id_), generation(0) {}
};
...
public:
    ...
    // Allocate a new node
    Node* allocate(double x, double y, int ring_id = -1) {
        if (!free_list_.empty()) {
        ...
        nodes_.push_back(std::make_unique<Node>(x, y, ring_id));
